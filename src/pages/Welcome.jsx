...
// Horizontal padding, borders, icons and tap targets stay fixed px — a 44px
// button is 44px on every phone. Only the vertical rhythm flexes.
const GUTTER = 20;
const VGAP = "clamp(12px, 3vh, 32px)";
...
export default function Welcome() {
  ...
  const header = (
    <header style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "space-between", padding: `16px ${GUTTER}px 8px` }}>
      <Logo size="sm" />
  ...
  const footer = (
    <div style={{ position: "relative", zIndex: 2, padding: `0 ${GUTTER}px 16px` }}>
      {/* ZIP code checker */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: "clamp(10px, 2vh, 14px)", padding: "5px 5px 5px 14px", background: "rgba(8,26,54,0.72)", border: "1px solid rgba(105,190,255,0.4)", clipPath: "polygon(13px 0, 100% 0, 100% calc(100% - 13px), calc(100% - 13px) 100%, 0 100%, 0 13px)" }}>
      ...
      <div role="status" aria-live="polite" style={{ minHeight: 16, marginTop: 6, textAlign: "center", fontFamily: "var(--font-jetbrains)", fontWeight: 500, fontSize: 10.5, lineHeight: "16px", letterSpacing: "0.04em", color: "#8fb9e2" }}>
        {zipMsg}
      </div>
      ...
      <button
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: 44, marginTop: 10, background: "rgba(10,30,60,0.5)", backdropFilter: "blur(8px)", border: "1px solid rgba(120,190,255,0.32)", color: "#bcd9f5", fontFamily: "var(--font-chakra)", fontWeight: 600, fontSize: 11.5, lineHeight: 1, letterSpacing: "0.17em", textTransform: "uppercase", cursor: "pointer", clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)" }}
      >
  ...
  return (
    <AppShell header={header} footer={footer} background="#020710">
      {/* Background layers — fixed, behind the whole shell */}
      <div aria-hidden="true" style={{ position: "fixed", inset: 0, zIndex: 0, background: "radial-gradient(120% 78% at 50% 16%, #0d2a58 0%, #061428 46%, #020710 100%)" }} />
      <div aria-hidden="true" style={{ position: "fixed", top: 0, left: 0, right: 0, height: "clamp(300px, 46dvh, 440px)", zIndex: 0, maskImage: "linear-gradient(180deg, #000 0%, #000 58%, rgba(0,0,0,0) 100%)", WebkitMaskImage: "linear-gradient(180deg, #000 0%, #000 58%, rgba(0,0,0,0) 100%)" }}>
        <img
      ...
      {/* Cyber frame */}
      <div aria-hidden="true" style={{ position: "fixed", inset: 10, border: "1px solid rgba(86,180,255,0.16)", pointerEvents: "none", zIndex: 1 }} />
      <div aria-hidden="true" style={{ position: "fixed", top: 10, left: 10, width: 26, height: 26, borderLeft: "2px solid rgba(125,205,255,0.85)", borderTop: "2px solid rgba(125,205,255,0.85)", boxShadow: "-1px -1px 12px rgba(80,170,255,0.6)", pointerEvents: "none", zIndex: 1 }} />
      <div aria-hidden="true" style={{ position: "fixed", top: 10, right: 10, width: 26, height: 26, borderRight: "2px solid rgba(125,205,255,0.85)", borderTop: "2px solid rgba(125,205,255,0.85)", boxShadow: "1px -1px 12px rgba(80,170,255,0.6)", pointerEvents: "none", zIndex: 1 }} />
      <div aria-hidden="true" style={{ position: "fixed", bottom: 10, left: 10, width: 26, height: 26, borderLeft: "2px solid rgba(125,205,255,0.85)", borderBottom: "2px solid rgba(125,205,255,0.85)", boxShadow: "-1px 1px 12px rgba(80,170,255,0.6)", pointerEvents: "none", zIndex: 1 }} />
      <div aria-hidden="true" style={{ position: "fixed", bottom: 10, right: 10, width: 26, height: 26, borderRight: "2px solid rgba(125,205,255,0.85)", borderBottom: "2px solid rgba(125,205,255,0.85)", boxShadow: "1px 1px 12px rgba(80,170,255,0.6)", pointerEvents: "none", zIndex: 1 }} />
      <div aria-hidden="true" style={{ position: "fixed", left: 10, right: 10, top: 0, height: 1, background: "linear-gradient(90deg, transparent, rgba(130,210,255,0.55), transparent)", animation: "nxscan 9s linear infinite", pointerEvents: "none", zIndex: 1 }} />

      {loggedOut && (
        <div style={{ position: "fixed", zIndex: 30, top: "calc(env(safe-area-inset-top, 0px) + 12px)", left: GUTTER, right: GUTTER, padding: "10px 16px", background: "rgba(16, 80, 50, 0.55)", border: "1px solid rgba(80, 220, 140, 0.4)", borderRadius: 8, display: "flex", alignItems: "center", gap: 10, backdropFilter: "blur(12px)", boxShadow: "0 8px 28px rgba(1,6,14,0.6)", pointerEvents: "none" }}>
      ...
      {/* Content: radar + headline, centred, absorbing all spare height. The
          radar keeps its full size; if a short phone cannot fit it alongside
          the headline, this region scrolls rather than shrinking the radar. */}
      <div className="scrollbar-hide" style={{ position: "relative", zIndex: 2, flex: 1, minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", justifyContent: "center", padding: `0 ${GUTTER}px` }}>
        <LandingRadar />
        <div style={{ textAlign: "center", flex: "none", paddingTop: VGAP }}>
          <h1 style={{ margin: 0, fontFamily: "var(--font-chakra)", fontWeight: 700, fontStyle: "italic", fontSize: "clamp(30px, 7vw, 44px)", lineHeight: 1.06, letterSpacing: "0.02em", textTransform: "uppercase", textShadow: "0 0 20px rgba(90,180,255,0.6)" }}>
            Discover people
          </h1>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginTop: 2 }}>
            ...
            <div style={{ fontFamily: "var(--font-chakra)", fontWeight: 700, fontStyle: "italic", fontSize: "clamp(30px, 7vw, 44px)", lineHeight: 1.06, letterSpacing: "0.02em", textTransform: "uppercase", textShadow: "0 0 20px rgba(90,180,255,0.6)" }}>
              Nearby
            </div>
        ...
        <div style={{ flex: "none", paddingTop: VGAP }}>
      {/* Stat cards */}
    <div style={{ display: "flex", gap: 10 }}>
      ...
      {/* Location note */}
    <p style={{ margin: "4px 0 0", textAlign: "center", fontFamily: "var(--font-chakra)", fontWeight: 500, fontSize: 9.5, lineHeight: 1.7, letterSpacing: "0.15em", textTransform: "uppercase", color: "#8fb9e2" }}>
