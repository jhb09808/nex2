...
import { Mail, Copy, Check, Search, Download, Users, Clock } from "lucide-react";
...
export default function EmailListPanel({ waitlist, users }) {
  ...
  const handleDownloadCsv = () => {
    const header = "Email,Source,Status,ZIP,Signup Date\n";
    const rows = filtered
      .map((e) => `${e.email},${e.source},${e.status || ""},${e.zip_code || ""},${new Date(e.date).toLocaleDateString()}`)
      .join("\n");
